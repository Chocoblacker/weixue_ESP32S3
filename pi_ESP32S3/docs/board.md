...
### ⚠️ 坑：“看日志”这个动作本身就会复位板子

**ESP32-S3 的 USB-Serial-JTAG 把 DTR/RTS 的电平变化解释成复位请求。**
实测：用 `pyserial` 打开 `/dev/ttyACM0`（它一打开就把 DTR/RTS 拉高）
**3/3 次都出现 `ESP-ROM:esp32s3` 启动横幅** —— 板子被我复位了。
同理 `esptool`、`idf.py monitor` 打开端口都会复位。

后果：
- 调试按键 / 崩溃 / 复位原因时，**工具本身会破坏现场**（本来想抓的复位原因
  被我自己的复位顶掉了）。
- 正在跑对话或 OTA 时想偷看日志，一打开就把人家打断了。

正确做法：`os.open()` + termios 直接读 tty，**全程不碰 DTR/RTS**。
（`cat /dev/ttyACM0` 虽然也不碰，但收不到数据。）
`scripts/watch-serial.py` 就是这么实现的。

另一个小坑：驱动接收缓冲区里可能残留**上一次开机**的日志，
导致同一次读数里出现相差 73 秒的两个运行时长。
`watch-serial.py` 开头有 `--flush`（默认 2 秒）先吐掉陈旧数据。

### 区分“真断电”和“软复位” —— 看 USB 端口

| 现象 | 含义 |
| --- | --- |
| `/dev/ttyACM0` **消失**（usbipd 退回 `Shared`） | **真·断电**或硬件复位（设备重新枚举了） |
| 端口还在，日志里出现启动横幅 `rst:0x..` | 软复位（芯片复位，USB 不重新枚举） |
| 端口在、无启动横幅，只有业务日志 | 纯软件行为（按键事件） |

另一种判据：日志里的 `I (xxxxx)` 是**设备自己的运行时长**，单调递增。
它变小就说明芯片重启过。但必须用被动读法 + 预热丢弃，否则会被缓冲陈旧数据骗。
